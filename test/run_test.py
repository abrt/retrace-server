...
def create_repo(packages, releaseid, version):
    ...
    child = Popen(createrepo_cmd, stdout=PIPE, stderr=PIPE, encoding='utf-8')
...
def generate_coredump():
    ...
    child = Popen(gcore_cmd, stdout=PIPE, stderr=PIPE, encoding='utf-8')
    stdout, stderr = child.communicate()
    if child.returncode:
    ...
    child = Popen(rpm_cmdline, shell=True, stdout=PIPE, stderr=PIPE, encoding='utf-8')
...
if not dont_create_repo_arg:
    ...
    for part in missing_unparsed:
        cmd.append(part[0])
    child = Popen(cmd, stdout=PIPE, stderr=PIPE, encoding='utf-8')
